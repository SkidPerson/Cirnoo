...
--[[
    Editor App Module
    DataStore Editor - GUI fiel ao original (LMG2L)
    dentro da Window/Resize do Dex
]]
-- Common Locals
...
local function main()
    ...
    Editor.Init = function()
        -- ============================================
        -- WINDOW DO DEX (drag, resize, minimize, close)
        -- ============================================
        ...
        -- ScrollingFrame ocupa todo o content
        -- (header ja vem da Window do Dex)
        ...
        -- ============================================
        -- IMAGE BUTTON - replica do ImageButton_4
        -- toggle que mostra/esconde a lista
        -- ============================================
        ...
        toggleBtn.Image = [[rbxassetid://110844258094575]]
        toggleBtn.Size = UDim2.new(0, 12, 0, 12)
        toggleBtn.Position = UDim2.new(0, 2, 0, 4)
        toggleBtn.Parent = scrollFrame

        -- ============================================
        -- TEXT LABEL "PlayerData" - replica do TextLabel_5
        -- ============================================
        local playerDataLabel = Instance.new("TextLabel")
        playerDataLabel.Name = "PlayerDataLabel"
        playerDataLabel.BackgroundTransparency = 1
        playerDataLabel.BorderSizePixel = 0
        playerDataLabel.TextSize = 16
        playerDataLabel.FontFace = Font.new(
            [[rbxasset://fonts/families/SourceSansPro.json]],
            Enum.FontWeight.Regular,
            Enum.FontStyle.Normal
        )
        playerDataLabel.TextColor3 = Color3.fromRGB(222, 222, 222)
        playerDataLabel.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        playerDataLabel.Size = UDim2.new(0, 138, 0, 10)
        playerDataLabel.Position = UDim2.new(0, 60, 0, 6)
        playerDataLabel.TextXAlignment = Enum.TextXAlignment.Left
        playerDataLabel.Text = "PlayerData"
        playerDataLabel.Parent = scrollFrame

        -- ============================================
        -- LISTA DINAMICA DE PLAYERS
        -- replica do sistema do original com indice
        -- e posicoes identicas aos TextButton_6..9
        -- ============================================
        ...
        -- ============================================
        -- JANELA DE EDIÇÃO (abre ao clicar num player)
        -- baseada no layout LMG2L, dentro do Dex Window
        -- ============================================
        local editWindow = nil

        local function openEditWindow(playerName)
            ...
            -- ── Detecta tudo dentro do player ────────────────────
            -- Prioridade: Folder "Inventory" > qualquer Folder > ValueBase direto
            local valueObj   = nil   -- ValueBase encontrado (se não for inventário)
            local valueType  = nil   -- "inventory" | ClassName do ValueBase | nil
            local inventory  = nil   -- Folder/Model de itens
            local valuesList = {}    -- lista de {name, className, value} para o textbox

            -- 1. Pasta chamada "Inventory" explícita
            local invFolder = target:FindFirstChild("Inventory")
            if invFolder and (invFolder:IsA("Folder") or invFolder:IsA("Model")) then
                inventory = invFolder
                valueType = "inventory"
            else
                -- 2. Qualquer Folder/Model filho
                local folder = target:FindFirstChildWhichIsA("Folder")
                            or target:FindFirstChildWhichIsA("Model")
                if folder then
                    inventory = folder
                    valueType = "inventory"
                else
                    -- 3. ValueBase direto no player (StringValue, BoolValue, IntValue…)
                    local found = target:FindFirstChildWhichIsA("ValueBase")
                    if found then
                        valueObj  = found
                        valueType = found.ClassName
                    end
                end
            end

            -- ── Monta o texto do TextBox e a linha de info ────────
            local initialText = ""
            local infoText    = ""
            ...
            elseif valueObj then
                initialText = tostring(valueObj.Value)
                infoText    = valueObj.ClassName .. " · " .. valueObj.Name
            else
                initialText = "(sem dados)"
                infoText    = "Nenhum value encontrado em " .. playerName
            end
            ...
            editWindow:Resize(248, 116)   -- só o content; o Dex adiciona o header automaticamente
            Editor.EditWindow = editWindow

            local eContent = editWindow.GuiElems.Content

            -- ── Frame_3 do LMG2L: fundo escuro que ocupa tudo ────
            local bodyFrame = Instance.new("Frame")
            bodyFrame.Name             = "Frame_3"
            bodyFrame.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
            bodyFrame.BorderSizePixel  = 0
            bodyFrame.Size             = UDim2.new(1, 0, 1, 0)
            bodyFrame.Position         = UDim2.new(0, 0, 0, 0)
            bodyFrame.Parent           = eContent

            -- ── Label de info (tipo/nome) — substituindo TextLabel_8 ──
            -- TextLabel_8 era o "Edit Value" na barra de título do LMG2L;
            -- como o Dex já tem header, usamos ele aqui como info do objeto
            local infoLabel = Instance.new("TextLabel")
            infoLabel.Name                  = "InfoLabel"
            infoLabel.BackgroundTransparency = 1
            infoLabel.BorderSizePixel        = 0
            infoLabel.TextSize               = 12
            infoLabel.FontFace               = Font.new(
                [[rbxasset://fonts/families/SourceSansPro.json]],
                Enum.FontWeight.Regular,
                Enum.FontStyle.Normal
            )
            ...
            infoLabel.Parent         = bodyFrame

            -- ── TextLabel "Value:" — TextLabel_5 do LMG2L ────────
            local valueLabel = Instance.new("TextLabel")
            valueLabel.Name                  = "TextLabel_5"
            ...
            -- ── TextButton "Save" — TextButton_6 do LMG2L ────────
            local saveBtn = Instance.new("TextButton")
            saveBtn.Name             = "TextButton_6"
            saveBtn.BackgroundColor3 = Color3.fromRGB(69, 69, 69)
            saveBtn.BorderSizePixel  = 0
            saveBtn.TextSize         = 15
            saveBtn.TextColor3       = Color3.fromRGB(150, 150, 150)
            saveBtn.FontFace         = Font.new(
                [[rbxasset://fonts/families/SourceSansPro.json]],
                Enum.FontWeight.Regular,
                Enum.FontStyle.Normal
            )
            ...
            -- ── TextButton "Cancel" — TextButton_7 do LMG2L ──────
            local cancelBtn = Instance.new("TextButton")
            cancelBtn.Name             = "TextButton_7"
            cancelBtn.BackgroundColor3 = Color3.fromRGB(69, 69, 69)
            cancelBtn.BorderSizePixel  = 0
            cancelBtn.TextSize         = 15
            cancelBtn.TextColor3       = Color3.fromRGB(210, 210, 210)
            ...
            saveBtn.MouseButton1Click:Connect(function()
                ...
                if valueType == "inventory" and inventory then
                    -- Tenta extrair items do JSON: {"Inventory":["a","b"]} ou ["a","b"]
                    local inner = raw:match('"Inventory"%s*:%s*%[(.-)%]')
                                or raw:match("%[(.-)%]")
                                or raw
                    -- Destrói filhos antigos
                    for _, child in ipairs(inventory:GetChildren()) do
                        child:Destroy()
                    end
                    -- Recria como StringValues (um por item entre aspas)
                    local found = false
                    for item in inner:gmatch('"([^"]*)"') do
                        -- Ignora o par nome:valor (ex: "item:10") — usa só o nome
                        local itemName = item:match("^([^:]+)") or item
                        if itemName ~= "" then
                    ...
                    -- Fallback sem aspas: separado por vírgula
                    if not found then
                        for item in inner:gmatch("[^,]+") do
                            item = item:match("^%s*(.-)%s*$")
                            if item ~= "" then
                ...
                elseif valueObj then
                    -- Aplica conversão correta por tipo
                    if valueType == "BoolValue" then
                        valueObj.Value = (raw:lower() == "true" or raw == "1")
                    elseif valueType == "IntValue" then
                        valueObj.Value = math.floor(tonumber(raw) or valueObj.Value)
                    elseif valueType == "NumberValue"
                        or valueType == "DoubleConstrainedValue"
                        or valueType == "IntConstrainedValue" then
                        valueObj.Value = tonumber(raw) or valueObj.Value
                    elseif valueType == "Color3Value" then
                        -- Formato esperado: "R, G, B" (0–255)
                        local r, g, b = raw:match("(%d+)%s*,%s*(%d+)%s*,%s*(%d+)")
                        if r then
                            valueObj.Value = Color3.fromRGB(
                    ...
                    elseif valueType == "Vector3Value" then
                        ...
                        if x then
                            valueObj.Value = Vector3.new(
                                tonumber(x), tonumber(y), tonumber(z)
                            )
                        end
                    elseif valueType == "Vector2Value" then
                        local x, y = raw:match("([%-%.%d]+)%s*,%s*([%-%.%d]+)")
                        if x then
                            valueObj.Value = Vector2.new(tonumber(x), tonumber(y))
                        end
                    elseif valueType == "CFrameValue" then
                        -- Formato: "X,Y,Z, R00,R01,R02, R10,R11,R12, R20,R21,R22"
                        local nums = {}
                        for n in raw:gmatch("[%-%.%d]+") do
                            table.insert(nums, tonumber(n))
                        end
                        if #nums >= 12 then
                            valueObj.Value = CFrame.new(
                        ...
                        elseif #nums >= 3 then
                            valueObj.Value = CFrame.new(nums[1], nums[2], nums[3])
                        end
                    elseif valueType == "BrickColorValue" then
                        valueObj.Value = BrickColor.new(raw)
                    elseif valueType == "RayValue" then
                        -- Ray não é editável via texto simples; ignora
                    else
                        -- StringValue e qualquer outro
                        valueObj.Value = raw
                    end
                end
        ...
        local function recalcCanvas()
            local total = 24 + (#playerButtons * 18) + 10
            scrollFrame.CanvasSize = UDim2.new(0, 0, 0, math.max(total, 0))
        end

        local function createPlayerButton(playerName, isLocalPlayer, index)
            -- replica exata dos TextButton_6 / TextButton_7/8/9
            local btn = Instance.new("TextButton")
            btn.Name = "PlayerBtn_" .. playerName
            ...
            btn.Size = UDim2.new(0, 172, 0, 18)
            -- posicao identica ao original: X=78, Y=24+(index*18)
            btn.Position = UDim2.new(0, 78, 0, 24 + (index * 18))
            btn.Visible = false -- comeca escondido, igual ao original
            btn.Parent = scrollFrame

            -- LocalPlayer em dourado, outros em cinza claro
            if isLocalPlayer then
                btn.Text = playerName
                btn.TextColor3 = Color3.fromRGB(211, 211, 211)
            else
                btn.Text = playerName
                btn.TextColor3 = Color3.fromRGB(211, 211, 211)
            end

            -- Hover highlight sutil (só se não estiver selecionado)
            btn.MouseEnter:Connect(function()
                if btn.Visible and selectedBtn ~= btn then
            ...
            -- Clique: seleciona com fundo azul e abre janela de edição
            btn.MouseButton1Click:Connect(function()
                ...
                local target = Players:FindFirstChild(playerName)
                if target and Properties then
                    Properties.ShowInstance(target)
                end

                openEditWindow(playerName)
            end)
        ...
        local function updatePlayerList()
            ...
            -- 1. LocalPlayer primeiro (igual ao original)
            createPlayerButton(LocalPlayer.Name, true, index)
            index = index + 1

            -- 2. Demais players do servidor
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= LocalPlayer then
                    createPlayerButton(player.Name, false, index)
                    index = index + 1
                end
            end

            -- Aplica visibilidade atual a todos
            for _, btn in ipairs(playerButtons) do
                btn.Visible = isListVisible
            end

            recalcCanvas()
        end

        -- Atualiza lista quando player entra ou sai
        Players.PlayerAdded:Connect(updatePlayerList)
        Players.PlayerRemoving:Connect(updatePlayerList)

        -- Popula lista ao iniciar
        updatePlayerList()

        -- ============================================
        -- TOGGLE DO IMAGEBUTTON (igual ao original)
        -- mostra/esconde todos os botoes de player
        -- ============================================
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, Console, SaveInstance, ModelViewer, SettingsWindow, Editor--[[, SecretServicePanel]], DefaultSettings, Notebook, Serializer, Lib local ggv = getgenv or nil
local API, RMD
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","SecretServicePanel","SaveInstance","ModelViewer","SettingsWindow","Editor"}
	Main.Elevated = false
		...
		if env.readfile and listfiles then
			if #listfiles("dex/plugins") > 0 then
				...
				for _, pluginDir in pairs(listfiles("dex/plugins")) do
					if pluginDir:find("DataStoreEditor") then continue end
