...
--[[
    Editor App Module
    DataStore Editor — dentro da Window/Resize do Dex
    GUI de edição baseada no layout LMG2L original
    Suporta: Inventory (Folder), StringValue, BoolValue,
             IntValue, NumberValue, DoubleConstrainedValue,
             IntConstrainedValue, CFrameValue, Vector3Value,
             Color3Value, Ray, ObjectValue
]]

-- Common Locals
local Main,Lib,Apps,Settings
local Explorer, Properties, ScriptViewer, SecretServicePanel, Notebook
...
local function main()
    ...
    -- ──────────────────────────────────────────────────────────────────
    -- Helper: font padrão do Dex/LMG2L
    -- ──────────────────────────────────────────────────────────────────
    local function ssp(weight, style)
        return Font.new(
    ...
    -- ──────────────────────────────────────────────────────────────────
    -- Helper: detecta inventory/valueBase de um player
    -- Retorna: valueType, valueObj, inventory, initialText, infoText
    --   valueType = "inventory" | ClassName do ValueBase | nil
    -- ──────────────────────────────────────────────────────────────────
    local function detectPlayerData(target)
        local valueObj, valueType, inventory
        local initialText, infoText = "", "(nenhum value encontrado)"

        -- 1. Folder chamada "Inventory"
        local inv = target:FindFirstChild("Inventory")
        if inv and inv:IsA("Folder") then
            inventory = inv
            valueType = "inventory"
        else
            -- 2. Qualquer outra Folder
            local folder = target:FindFirstChildWhichIsA("Folder")
            if folder then
                inventory = folder
                valueType = "inventory"
            else
                -- 3. ValueBase direto no player
        ...
        if valueType == "inventory" and inventory then
            ...
            initialText = '{"Inventory":[' .. table.concat(quoted, ",") .. "]}"
            infoText    = inventory.Name .. "  ·  " .. #items .. " item(s)"
        elseif valueObj then
            initialText = tostring(valueObj.Value)
            infoText    = valueObj.ClassName .. "  ·  " .. valueObj.Name
        end

        return valueType, valueObj, inventory, initialText, infoText
    end

    -- ──────────────────────────────────────────────────────────────────
    -- Abre (ou reabre) a janela de edição ao clicar num nome de player
    -- ──────────────────────────────────────────────────────────────────
    ...
    local function openEditWindow(playerName)
        ...
        local valueType, valueObj, inventory, initialText, infoText =
            detectPlayerData(target)

        -- ── Calcula altura da janela ────────────────────────────────
        -- Modo inventário: mostra lista de itens + TextBox + botões
        -- Modo value: só TextBox + botões
        local isInv     = (valueType == "inventory" and inventory ~= nil)
        local invItems  = {}
        if isInv then
            for _, child in ipairs(inventory:GetChildren()) do
        ...
        -- altura: header(20) + infobar(22) + [lista itens] + valueLine(30) + textbox(24) + botões(36) + padding(8)
        local listH   = isInv and math.max(#invItems * 18, 18) or 0
        local totalH  = 22 + listH + 30 + 24 + 36 + 14
        totalH = math.max(totalH, 136)

        -- ── Window Dex ──────────────────────────────────────────────
        editWindow = Lib.Window.new()
        editWindow:SetTitle("Edit Value — " .. playerName)
        editWindow:Resize(248, totalH)

        local eContent = editWindow.GuiElems.Content

        -- Frame raiz (Frame_3 do LMG2L): fundo escuro
        ...
        local cursorY = 6  -- cursor de posicionamento vertical

        -- ── Barra de info (tipo · nome / contagem) ──────────────────
        -- Fundo levemente diferente, igual ao Frame_2 do LMG2L (58,58,58)
        local infoBar = Instance.new("Frame")
        infoBar.BackgroundColor3 = Color3.fromRGB(58, 58, 58)
        infoBar.BorderSizePixel  = 0
        infoBar.Size             = UDim2.new(1, 0, 0, 20)
        infoBar.Position         = UDim2.new(0, 0, 0, cursorY)
        infoBar.Parent           = bodyFrame

        local infoLabel = Instance.new("TextLabel")
        infoLabel.BackgroundTransparency = 1
        infoLabel.BorderSizePixel        = 0
        infoLabel.TextSize               = 13
        infoLabel.FontFace               = ssp()
        ...
        cursorY = cursorY + 20 + 4

        -- ── Lista visual de itens do inventário ─────────────────────
        -- Só aparece quando valueType == "inventory"
        if isInv then
            ...
            listFrame.Size                = UDim2.new(1, -4, 0, listH)
            listFrame.Position            = UDim2.new(0, 2, 0, cursorY)
            listFrame.CanvasSize          = UDim2.new(0, 0, 0, #invItems * 18)
            listFrame.Parent              = bodyFrame

            for i, itemName in ipairs(invItems) do
                ...
                row.Position               = UDim2.new(0, 6, 0, (i - 1) * 18)
                row.Text                   = "• " .. itemName
                row.Parent                 = listFrame
            end

            cursorY = cursorY + listH + 4
        end

        -- ── Linha "Value:" + TextBox ────────────────────────────────
        -- TextLabel_5 do LMG2L: "Value:" — NÃO muda
        local valueLabel = Instance.new("TextLabel")
        valueLabel.BackgroundTransparency = 1
        valueLabel.BorderSizePixel        = 0
        valueLabel.TextSize               = 15
        valueLabel.FontFace               = ssp()
        valueLabel.TextColor3             = Color3.fromRGB(255, 255, 255)
        valueLabel.Size                   = UDim2.new(0, 50, 0, 24)
        valueLabel.Position               = UDim2.new(0, 0, 0, cursorY + 4)
        ...
        valueBox.FontFace         = ssp()
        valueBox.Size             = UDim2.new(0, 186, 0, 18)
        valueBox.Position         = UDim2.new(0, 54, 0, cursorY + 6)
        valueBox.Text             = initialText
        valueBox.Parent           = bodyFrame

        cursorY = cursorY + 30

        -- ── Botões Save / Cancel (TextButton_6 e _7 do LMG2L) ───────
        local btnY = cursorY + 6
        ...
        cancelBtn.MouseEnter:Connect(function()
            cancelBtn.BackgroundColor3 = Color3.fromRGB(85, 85, 85)
        end)
        ...
        saveBtn.MouseButton1Click:Connect(function()
            ...
            if isInv and inventory then
                -- Tenta parsear JSON-like: {"Inventory":["a","b"]} ou ["a","b"]
                local inner = raw:match('"Inventory"%s*:%s*%[(.-)%]')
                           or raw:match("%[(.-)%]")
                           or raw
                -- Destrói filhos antigos e recria
                for _, child in ipairs(inventory:GetChildren()) do child:Destroy() end
                local found = false
                ...
                -- Fallback: lista simples separada por vírgula sem aspas
                if not found then
                    for item in inner:gmatch("[^,]+") do
                        item = item:match("^%s*(.-)%s*$")
                        if item ~= "" then
            ...
            elseif valueObj then
                ...
                elseif valueType == "Vector3Value" then
                    ...
                    if x then valueObj.Value = Vector3.new(tonumber(x), tonumber(y), tonumber(z)) end
                elseif valueType == "Color3Value" then
                    -- espera "r, g, b" em 0-255
                    local r, g, b = raw:match("([%d%.]+)%s*,%s*([%d%.]+)%s*,%s*([%d%.]+)")
                    if r then
                        local rv, gv, bv = tonumber(r), tonumber(g), tonumber(b)
                        -- aceita 0-1 ou 0-255
                        if rv > 1 or gv > 1 or bv > 1 then
                            valueObj.Value = Color3.fromRGB(rv, gv, bv)
                        else
                            valueObj.Value = Color3.new(rv, gv, bv)
                        end
                    end
                else
                    -- StringValue, ObjectValue (pelo nome), etc.
                    valueObj.Value = raw
                end
            end
    ...
    -- ──────────────────────────────────────────────────────────────────
    -- Inicialização: window principal do Editor
    -- ──────────────────────────────────────────────────────────────────
    Editor.Init = function()
        ...
        -- ScrollingFrame que ocupa o content inteiro
        ...
        -- Seta/toggle — ImageButton_4 do LMG2L
        ...
        -- Label "PlayerData" — TextLabel_5 do LMG2L
        ...
        local function createPlayerButton(playerName, isLocalPlayer, index)
            ...
            btn.Visible           = false
            btn.Text              = playerName
            btn.TextColor3        = isLocalPlayer
                and Color3.fromRGB(255, 215, 100)   -- local player: dourado
                or  Color3.fromRGB(211, 211, 211)   -- outros: cinza
            ...
            -- ── CLIQUE: abre a janela de edição ───────────────────
            btn.MouseButton1Click:Connect(function()
                ...
                -- Mostra o player no painel Properties do Dex (opcional)
                local target = Players:FindFirstChild(playerName)
